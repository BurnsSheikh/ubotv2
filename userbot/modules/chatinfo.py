...
async def info(event):
    await event.edit("__🔄Recupero le info...__")
...
async def get_chatinfo(event):
    ...
    try:
        ...
        try:
            ...
            await event.edit("⚠️ Specificare la chat.")
            return None
        except ChannelPrivateError:
            await event.edit("""⚠️ Gruppo/Canale non trovato!
__Se questo gruppo/canale è privato o sono bannato non posso recuperare le informazioni.__""")
            return None
        except ChannelPublicGroupNaError:
            await event.edit("⚠️ Chat Inesistente")
            return None
        except (TypeError, ValueError) as err:
            await event.edit("⚠️ Errore")
            return None
    return chat_info
...
async def fetch_info(chat, event):
    ...
    bots = 0
    supergroup = "<b>✅</b>" if hasattr(chat_obj_info, "megagroup") and chat_obj_info.megagroup else "No"
    ...
    caption = "<b>⚙️ CHAT INFO</b>\n\n"
    if former_title is not None:  # Meant is the very first title
        caption += f"📬Nome : {former_title}\n"
    caption += f"🆔Chat ID : <code>-100{chat_obj_info.id}</code>\n"
    if members is not None:
        caption += f"👥Membri: : <code>{members}</code>\n"
    if members_online:
        caption += f"✅Membri online:: <code>{members_online}</code>\n"
    if not broadcast:
        caption += f"🔰Supergruppo : {supergroup}\n"
    if username is not None:
        caption += f"🔐Privacy : Pubblico\n"
        caption += f"🔗Link : {username}\n"
    else:
        caption += f"🔐Privacy : Privato\n"
    if messages_viewable is not None:
        caption += f"👀Messaggi visibili : <code>{messages_viewable}</code>\n"
    if messages_sent:
        caption += f"💬Messaggi totali: : <code>{messages_sent}</code>\n"
    if creator_username is not None:
        caption += f"👑Proprietario : {creator_username}\n"
    elif creator_valid:
        caption += f"👑Proprietario : <a href=\"tg://user?id={creator_id}\">{creator_firstname}</a>\n"
    if admins is not None:
        caption += f"👮🏻‍♂️Amministratori : <code>{admins}</code>\n"
    if bots_list:
        caption += f"🤖Bot : <code>{bots}</code>\n"
    if banned_users is not None:
        caption += f"🚫Bannati : <code>{banned_users}</code>\n"
    ...
    if group_stickers is not None:
        caption += f"🎁Pacchetto sticker : <a href=\"t.me/addstickers/{chat.full_chat.stickerset.short_name}\">{group_stickers}</a>\n"
    if description:
        caption += f"📓Biografia gruppo : <code>{description}</code>\n"
    return caption
